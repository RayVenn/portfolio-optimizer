...
class TransactionCosts(BaseModel):
    ...
    flat: Optional[float] = Field(
        default=None,
        description="Flat transaction cost per asset traded"
    )
...
class PortfolioConstraints(BaseModel):
    ...
    weight_bounds: Optional[list[tuple[float, float]]] = Field(
        default=None,
        description="Per-asset weight bounds as list of (min, max) tuples"
    )
    ...
    sector_lower: Optional[dict[str, float]] = Field(
        default=None,
        description="Minimum weight per sector (e.g., {'Tech': 0.1})"
    )
    sector_upper: Optional[dict[str, float]] = Field(
        default=None,
        description="Maximum weight per sector (e.g., {'Tech': 0.4})"
    )
...
class PortfolioRequest(BaseModel):
    ...
    covariance_matrix: list[list[float]] = Field(..., description="Covariance matrix of returns")
    ...
    target_volatility: Optional[float] = Field(default=None, description="Target volatility for optimization")
    transaction_costs: Optional[TransactionCosts] = Field(default=None, description="Transaction cost model")
