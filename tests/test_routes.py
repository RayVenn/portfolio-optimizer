...
class TestMaxSharpeEndpoint:
    def test_successful_optimization(self, client, sample_portfolio_data):
    ...
    def test_with_constraints(self, client, sample_portfolio_data):
        request_data = {
            **sample_portfolio_data,
            "constraints": {"min_weight": 0.1, "max_weight": 0.4},
        }
        response = client.post("/api/v1/optimize/max-sharpe", json=request_data)
        assert response.status_code == 200
    ...
    def test_with_transaction_costs(self, client, sample_portfolio_data):
        request_data = {
            **sample_portfolio_data,
            "transaction_costs": {
                "proportional": 0.001,
    ...
    def test_invalid_request(self, client):
        response = client.post("/api/v1/optimize/max-sharpe", json={})
...
class TestMinVolatilityEndpoint:
    def test_successful_optimization(self, client, sample_portfolio_data):
        response = client.post(
            "/api/v1/optimize/min-volatility", json=sample_portfolio_data
        )
        assert response.status_code == 200
    ...
    def test_infeasible_constraints(self, client):
        ...
        response = client.post("/api/v1/optimize/min-volatility", json=request_data)
        assert response.status_code == 400

    def test_with_sector_constraints(self, client):
        request_data = {
...
class TestEfficientReturnEndpoint:
    def test_successful_optimization(self, client, sample_portfolio_data):
        ...
        response = client.post("/api/v1/optimize/efficient-return", json=request_data)
        assert response.status_code == 200

        data = response.json()
        assert abs(data["expected_return"] - 0.11) < 0.01

    def test_without_target_return(self, client, sample_portfolio_data):
        response = client.post(
            "/api/v1/optimize/efficient-return", json=sample_portfolio_data
        )
        assert response.status_code == 400

    def test_infeasible_target_return(self, client, sample_portfolio_data):
...
class TestEfficientRiskEndpoint:
    def test_successful_optimization(self, client, sample_portfolio_data):
        ...
        response = client.post("/api/v1/optimize/efficient-risk", json=request_data)
        assert response.status_code == 200

        data = response.json()
        assert abs(data["volatility"] - 0.18) < 0.02

    def test_without_target_volatility(self, client, sample_portfolio_data):
        response = client.post(
            "/api/v1/optimize/efficient-risk", json=sample_portfolio_data
        )
        assert response.status_code == 400


class TestEdgeCases:
    def test_two_asset_portfolio(self, client):
        request_data = {
            "tickers": ["A", "B"],
    ...
    def test_single_asset_portfolio(self, client):
        request_data = {
            "tickers": ["A"],
    ...
    def test_negative_expected_returns(self, client):
        request_data = {
            "tickers": ["A", "B", "C"],
    ...
    def test_custom_risk_free_rate(self, client, small_portfolio_data):
        request_data = {**small_portfolio_data, "risk_free_rate": 0.05}
        response = client.post("/api/v1/optimize/max-sharpe", json=request_data)
        assert response.status_code == 200

    def test_allowing_short_selling(self, client):
        request_data = {
            "tickers": ["A", "B", "C"],
